...
RequestHandler.prototype.sendError = function(errorMessage, userCb) {
  ...
  function doRequest() {
    that._request({
      url: getErrorReportURL(that._projectId, that._key),
      method: 'POST',
      json: errorMessage
    }, cb);
  }
  ...
  if (!that._projectId) {
    commonDiag.utils.getProjectNumber(function(err, projectId) {
      // FIXME(ofrobots): This behaviour is different from the debug and trace
      // agents. debug doesn't check GCLOUD_PROJECT until it has failed to fetch
      // the projectId from the metadata service. This should be reconclied
